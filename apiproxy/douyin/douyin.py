...
import time
import copy
from apiproxy.common.http_client import http_get, http_post
...
class Douyin(object):
    ...
    def getKey(self, url: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            r = http_get(url=url, headers=douyin_headers)
        ...
        if "/user/" in urlstr:
            ...
            res = http_get(url, headers=douyin_headers)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    response = http_get(url=jx_url, headers=douyin_headers, timeout=10)
        ...
        with Progress(
            ...
            while True:
                try:
                    ...
                    # 发送请求
                    res = http_get(url=url, headers=douyin_headers, timeout=10)
    ...
    _WEB_PARAMS = (
        ...
        '&browser_platform=MacIntel&browser_name=Chrome&browser_version=122.0.0.0'
    ...
    _COLLECTS_REFERER = 'https://www.douyin.com/user/self?showTab=favorite_collection'

    def _collects_headers(self):
        headers = douyin_headers.copy()
        headers['referer'] = self._COLLECTS_REFERER
        return headers

    def _fetch_paged_aweme_list(self, url_builder, data_key='aweme_list', cursor_key='cursor',
                                 has_more_key='has_more', number=0, label='作品'):
        """通用分页拉取 aweme 列表"""
        cursor = 0
        count = 35
        aweme_list = []
        headers = self._collects_headers()

        while True:
            url = url_builder(cursor, count)
            try:
                res = http_get(url=url, headers=headers, timeout=10)
                if res.status_code != 200:
                    self.console.print(f"[red]❌ HTTP {res.status_code}: {label}[/]")
                    break
                datadict = json.loads(res.text)
            except json.JSONDecodeError as e:
                self.console.print(f"[red]❌ JSON解析失败({label}): {e}[/]")
                break
            except Exception as e:
                self.console.print(f"[red]❌ 请求失败({label}): {e}[/]")
                break

            if not datadict or datadict.get('status_code') not in (0, None):
                msg = datadict.get('status_msg', '未知错误') if datadict else '空响应'
                self.console.print(f"[red]❌ API失败({label}): {msg}[/]")
                if not res.text.strip():
                    self.console.print("[yellow]💡 请检查 Cookie 是否有效并已登录[/]")
                break

            items = datadict.get(data_key) or []
            for aweme in items:
                if number > 0 and len(aweme_list) >= number:
                    return aweme_list
                converted = self._convert_aweme_data(aweme)
                if converted:
                    aweme_list.append(converted)

            if not datadict.get(has_more_key, False):
                break
            cursor = datadict.get(cursor_key, 0)
            if cursor == 0 and not items:
                break

        return aweme_list

    def getCollectsList(self, count=20, number=0):
        """获取当前登录用户的收藏夹列表"""
        cursor = 0
        collects = []
        headers = self._collects_headers()
        ...
        while True:
            params = f'cursor={cursor}&count={count}&{self._WEB_PARAMS}'
            url = self.urls.USER_COLLECTS_LIST + utils.getXbogus(params)
            try:
                res = http_get(url=url, headers=headers, timeout=10)
                datadict = json.loads(res.text)
            except Exception as e:
                ...
                if 'proxy' in err.lower():
                    self.console.print("[red]网络请求失败: 系统代理不可用[/]")
                    self.console.print("[yellow]提示: 可关闭 VPN/代理软件，或在终端执行:[/]")
                    self.console.print("[yellow]  set HTTP_PROXY=[/]")
                    self.console.print("[yellow]  set HTTPS_PROXY=[/]")
                else:
                    self.console.print(f"[red]获取收藏夹列表失败: {e}[/]")
                break

            if not datadict or datadict.get('status_code') != 0:
                msg = datadict.get('status_msg', '未知错误') if datadict else '空响应'
                self.console.print(f"[red]❌ 收藏夹列表 API 失败: {msg}[/]")
                break

            batch = datadict.get('collects_list') or []
            collects.extend(batch)
            if number > 0 and len(collects) >= number:
                return collects[:number]
    ...
    def getCollectsVideoList(self, collects_id, count=35, number=0):
        """获取指定收藏夹内的视频列表"""
        if not collects_id:
            return []

        def url_builder(cursor, page_count):
            params = (
                f'collects_id={collects_id}&cursor={cursor}&count={page_count}'
                f'&{self._WEB_PARAMS}'
            )
            return self.urls.USER_COLLECTS_VIDEO + utils.getXbogus(params)

        return self._fetch_paged_aweme_list(
            url_builder,
            label=f'收藏夹 {collects_id}',
            number=number,
        )

    def getFavoriteVideoList(self, count=20, number=0):
        """获取当前登录用户「收藏」页的全部收藏视频（非收藏夹）"""
        cursor = 0
        # listcollection 接口 count>20 时会返回 status_code=5
        page_count = min(count or 20, 20)
        aweme_list = []
        headers = self._collects_headers()
        label = '收藏视频'

        while True:
            params = f'{self._WEB_PARAMS}&publish_video_strategy_type=2'
            form_body = f'count={page_count}&cursor={cursor}'
            url = self.urls.USER_COLLECTION + utils.getXbogus(params, form=form_body)
            try:
                res = http_post(url=url, headers=headers, data=form_body, timeout=10)
                if res.status_code != 200:
                    self.console.print(f"[red]HTTP {res.status_code}: {label}[/]")
                    if aweme_list:
                        break
            ...
            status_code = datadict.get('status_code') if datadict else None
            if not datadict or status_code not in (0, None):
                msg = datadict.get('status_msg') if datadict else None
                detail = msg or f'status_code={status_code}'
                if aweme_list:
                    logger.warning('收藏视频分页中断(%s)，已保留 %d 条', detail, len(aweme_list))
                    break
                self.console.print(f"[red]API失败({label}): {detail}[/]")
                if status_code == 5:
                    self.console.print("[yellow]提示: Cookie 可能过期，请重新运行 cookie_extractor.py[/]")
                break

            items = datadict.get('aweme_list') or []
            for aweme in items:
                if number > 0 and len(aweme_list) >= number:
                    return aweme_list
                converted = self._convert_aweme_data(aweme)
    ...
    def _convert_aweme_data(self, aweme):
        ...
        try:
            ...
            result = copy.deepcopy(self.result.awemeDict)
            duration_ms = aweme.get('duration') or (aweme.get('video') or {}).get('duration') or 0
            result['duration_ms'] = int(duration_ms)
            result['duration_sec'] = round(int(duration_ms) / 1000, 1) if duration_ms else 0
            return result
        except Exception as e:
    ...
    def getLiveInfo(self, web_rid: str):
        ...
        while True:
            ...
            try:
                ...
                response = http_get(live_api, headers=douyin_headers)
    ...
    def getUserDetailInfo(self, sec_uid):
        ...
        while True:
            ...
            try:
                ...
                res = http_get(url=url, headers=douyin_headers)
                datadict = json.loads(res.text)
