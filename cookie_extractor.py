...
try:
    ...
    from utils.browser_helper import find_chrome_executable, get_chromium_launch_kwargs
except ImportError:
    print("请安装必要的依赖: pip install playwright rich pyyaml")
    print("若仍提示缺少浏览器，请确认已安装 Google Chrome")
    sys.exit(1)
...
class CookieExtractor:
    ...
    async def extract_cookies(self, headless: bool = False) -> Dict:
        ...
        async with async_playwright() as p:
            ...
            if chrome_path:
                console.print(f"[dim]使用系统 Chrome: {chrome_path}[/dim]")
            else:
                console.print("[dim]未找到 Chrome 路径，尝试 channel=chrome[/dim]")

            try:
                browser = await p.chromium.launch(
                    **get_chromium_launch_kwargs(headless=headless)
                )
            except Exception as launch_err:
                console.print(f"[red]启动 Chrome 失败: {launch_err}[/red]")
                console.print("[yellow]可选方案:[/yellow]")
                console.print("  1. 确认 Chrome 已安装")
                console.print("  2. 设置环境变量 CHROME_PATH 指向 chrome.exe")
                console.print("  3. 重新运行并选择 3（手动输入 Cookie）")
                console.print("  4. 或执行: playwright install chromium")
                return {}
            ...
            try:
                ...
                opened = await self._open_douyin_page(page)
                if not opened:
                    console.print("[red]无法打开抖音页面，请检查网络后重试[/red]")
                    return {}

                console.print("[green]页面已打开[/green]")
    ...
    async def _open_douyin_page(self, page: Page) -> bool:
        """Open Douyin homepage. Avoid networkidle (Douyin keeps polling)."""
        ...
        for wait_until in ('domcontentloaded', 'commit'):
            try:
                ...
                if 'douyin.com' in (page.url or ''):
                    console.print("[yellow]页面已打开（未完全加载），可直接登录[/yellow]")
                    return True
                console.print(f"[dim]加载失败 ({wait_until}): {e}[/dim]")
        return 'douyin.com' in (page.url or '')

    async def _has_login_cookie(self, page: Page) -> bool:
    ...
    async def _wait_for_login(self, page: Page, timeout: int = 300) -> bool:
        ...
        while time.time() - start_time < timeout:
            try:
                if await self._has_login_cookie(page):
                    await asyncio.sleep(2)
                    return True
...
async def main():
    ...
    if cookies:
        ...
        console.print("python download_collects.py")
    else:
        console.print("\n[red]❌ 未能提取Cookie[/red]")
